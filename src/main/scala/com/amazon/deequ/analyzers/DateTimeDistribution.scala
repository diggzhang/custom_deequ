...
//  def computeStateFromResult(result: Map[Timestamp, Timestamp], frequency: Long):
//  Map[(Timestamp, Timestamp), Long] = {
//    result.map({
//      case (x, y) => (new Timestamp(x.getTime), new Timestamp(x.getTime + frequency - 1L)) -> y.getTime
//    })
//  }
  ...
  Option[DateTimeDistributionState] = {
    ifNoNullsIn(result, offset) { _ =>
      DateTimeDistributionState(
        DateTimeDistributionState.computeStateFromResult(Map.empty[Long, Long] ++ result.getMap(2),
